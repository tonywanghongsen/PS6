...
public class PersonDAL {
	...
	/**
	 * addPerson - Method adds a student to the database
	 * @param stu
	 * @return
	 */
	public static PersonDomainModel addPerson(PersonDomainModel per) {
		Session session = HibernateError.getSessionFactory().openSession();
		Transaction tx = null;
		int employeeID = 0;
		try {
	...
	public static ArrayList<PersonDomainModel> getPerson() {
		...
		try {
			...
			List person = session.createQuery("FROM PersontDomainModel").list();
